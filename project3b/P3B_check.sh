...
do
	...
	do
		f=$pfx$b.$s
		curl -L -o $f $PROJECT_URL/$f > /dev/null 2> /dev/null
		ret=$?
		# curl returns 0 even if it gets an error response
		if [ $ret -eq 0 ]; then
		    grep "404 Not Found\|Error 404" $f > /dev/null
		    if [ $? -eq 0 ]; then
		        ret=1
		    fi
		fi
	...
	$TIMEOUT ./$PGM $pfx$b.csv | sort > TEST.out
